...
这是全组唯一一份评分代码。LLM 线 (run.py) 和 VLM 线 (evaluate.py)
...
"对比"环节其实不是在同一把尺子下比较。本文件把这些全部收回到一处：
  ...
  - yes/no 题：用本文件里唯一一份 parse_yes_no 解析，跟 eval_set 里的
    ground_truth 比较。
  - inference 题：不再用规则/bigram 自动判分，统一读 judge.py 产出的
...
输入文件：
  ...
  2) llm_direct.json / llm_cot.json / vlm_direct.json / vlm_cot.json
       —— 四条线各自的原始逐题记录（run.py / evaluate.py 的输出），
          每条记录至少有 case_id / question_id / qtype / raw_output。
          也可以直接传 run.py 的 raw_results.json（同时含 DA 和 CoT），
          用 --llm_results / --vlm_results，按 prompt_key 自动拆开。
  3) --judged_llm / --judged_vlm（可选）—— judge.py 产出的
...
产出（写到 out_dir/）：
    summary_2x2_comparison.json —— 2×2 准确率表 + McNemar 结果（yes/no 和 inference 都算）
...
def derive_qid(case_id, question):
    """Same fallback id scheme as judge.py's normalize_record(): when a
    ...
    judge.py, which uses this same scheme) join back to these records."""
...
def load_run(path, filter_prompt_key=None):
    ...
    raw_results.json from run.py), in which case filter_prompt_key picks
...
def load_judged(path):
    """Load judge.py output -> {(question_id, condition): judgment dict}."""
    if not path:
        return {}
    data = json.load(open(path, encoding="utf-8"))
    if not isinstance(data, list):
...
def score_run(eval_set, run, judged, condition, missing_policy):
    ...
    yes/no 用 parse_yes_no；inference 用 judged（judge.py 输出），
...
def main():
    ...
    ap.add_argument("--judged_llm", default=None, help="judge.py output for the LLM line (covers both DA and CoT)")
    ap.add_argument("--judged_vlm", default=None, help="judge.py output for the VLM line (covers both prompt conditions)")
    ...
    if not judged_llm and not judged_vlm:
        print("[WARN] No --judged_llm/--judged_vlm given. Inference questions will be reported as "
              "not_judged (run judge.py first to score them).")

    # The judged file's "condition" field is whatever prompt_key value was on
    # the raw record (see judge.py's CONDITION_KEYS), so it must match
    ...
    json.dump(summary, open(os.path.join(args.out_dir, "summary_2x2_comparison.json"), "w", encoding="utf-8"),
