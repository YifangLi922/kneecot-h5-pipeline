# When Does Chain-of-Thought Help? — H5 LLM and VLM Evaluation Pipeline
...
```mermaid
%%{init: {'theme': 'dark', 'themeVariables': {'primaryColor': '#1e293b', 'primaryTextColor': '#f1f5f9', 'primaryBorderColor': '#475569', 'lineColor': '#94a3b8', 'secondaryColor': '#0f172a', 'tertiaryColor': '#1e293b', 'background': '#000000', 'mainBkg': '#1e293b', 'nodeBorder': '#475569', 'clusterBkg': '#0f172a', 'titleColor': '#f1f5f9', 'edgeLabelBackground': '#0f172a', 'nodeTextColor': '#f1f5f9'}}}%%
flowchart LR
...
.nii MRI volumes])
    ...
    subgraph LEAK[Case Loading & Leakage Prevention]
        B1([✅ Used as input
Case ID
MR Findings · QA Pairs])
        B2([❌ Excluded — leakage
Diagnostic Impression
Structured Labels])
    end

    subgraph FILT[Task Filtering & Balanced Sampling · seed=42]
        C1[Keep yes/no + inference Qs
Require clean Yes/No GT
Remove non-knee joints
Balance Yes/No · fixed seed]
    end

    subgraph LLM[Text-only LLM Pipeline]
        D1[Input: MR Findings + question
no image · no impression · no labels]
        D2[Prompt: Direct / 4-step CoT
Chinese templates · 答案 marker]
        D3([Qwen2.5-7B-Instruct
greedy decoding])
        D1 --> D2 --> D3
    end

    subgraph VLM[Multimodal VLM Pipeline]
        E1[Input: MR Findings + question
+ 10 sagittal slices → 2×5 PNG grid]
        E2[Prompt: Direct / 4-step CoT
same text evidence as LLM]
        E3([🖼 Qwen2.5-VL 7B
greedy decoding])
        E1 --> E2 --> E3
    end
...
EN yes/no → CN 是/否 → UNCLEAR]
        F2[Inference 推理 questions
extract final conclusion
GT / LLM-as-judge → correct/incorrect]
    end

    G([Evaluation & Comparison
Yes/No accuracy · Inference accuracy
per task type · McNemar test])
...
notes/
└── coding problems and solutions.md   # internal dev notes (not for final submission)
...
│   ├── compare.py                  # yes/no accuracy + McNemar (RQ1/RQ2/RQ3)
│   ├── judge.py                    # LLM-as-judge for inference questions
│   ├── inference_rubric_for_LLM_judge.json   # rubric used by judge.py
│   ├── vlm_findings_ablation.py    # image-only vs image+findings ablation
│   └── sample_grounding_traces.py  # seed=42 sampler for grounding analysis (§3.4)
│
├── kneecot-h5-pipeline-llm/
│   ├── data/sample/                # small sample case for smoke-testing
│   ├── src/
│   │   ├── preprocessing.py        # filter knee-only cases, build eval items
│   │   ├── prompts.py              # DA and CoT prompt templates
│   │   ├── inference.py            # loads Qwen2.5-7B-Instruct, greedy generation
│   │   └── evaluation.py           # raw-output I/O only
│   ├── tests/                      # sanity tests
│   ├── run.py                      # end-to-end LLM runner
│   └── requirements.txt
│
└── kneecot-h5-pipeline-vlm/
    ├── config.py                   # paths, model toggle, CONDITIONS
    ├── preprocessing.py             # stitch MRI slices into grid PNG
    ├── build_eval_set.py           # builds shared eval_set.json
    ├── prompts.py                   # DA / CoT / DA_findings / CoT_findings
    ├── evaluate.py                  # VLM inference via local Ollama
    ├── run.py                       # preprocess -> build_eval_set -> evaluate
    ├── metrics.py                   # legacy/unused
    ├── VLM.ipynb                    # original Colab prototype
    └── requirements.txt

results/
├── compare_out/                    # output of compare.py
│   ├── summary_2x2_comparison.json # 2x2 accuracy + McNemar, per qtype
│   ├── summary_vlm_ablation.json   # RQ3 image-only ablation
│   ├── per_item.csv                # per-question outcome, all 4 conditions
│   ├── rq3_yesno.csv               # LLM-vs-VLM comparison under CoT
│   └── rq3_inference.csv
├── grounding_annotation_completed.csv   # 40-row A/B/C grounding labels (§3.4)
├── judged_inference_llm.json / judged_inference_vlm.json   # judge.py output, post-correction
├── manual_review_llm_judge_score_internal.jsonl / manual_review_vlm_judge_score_internal.jsonl    # Internal judge-scored pools; not shown to human reviewers
├── manual_review_llm_blind_for_reviewers.jsonl / manual_review_vlm_blind_for_reviewers.jsonl   # Blind human-review input files
└── full_human_review_LLM.csv / full_human_review_VLM.csv    # Human + judge verdicts combined; source of corrections in judged_inference_*.json
```
...
3. All `unclear` and `incorrect` verdicts, plus a random ~15–20% sample of judge-marked `correct` cases, are written to a manual-review file (`manual_review_llm.jsonl` / `manual_review_vlm.jsonl`). Two reviewers independently re-label this shared sample — their per-row verdicts and agreement with the judge are recorded in `full_human_review_LLM.csv` / `full_human_review_VLM.csv` — to measure human-judge agreement, and any judge errors found are corrected directly in `judged_inference_llm.json` / `judged_inference_vlm.json` before `compare.py` is run.
...
**Measured judge reliability.** Across the 136 manually-adjudicated items in `full_human_review_LLM.csv` / `full_human_review_VLM.csv` (the full `random_correct_spotcheck` sample plus a stratified sample of `boundary_case` items), human reviewers agreed with the judge on 132/136 (**97.06%**, Cohen's κ = **0.94**). The 4 disagreements (1 LLM, 3 VLM, all judge-said-`correct`/human-said-`incorrect`) were corrected directly in `judged_inference_llm.json` / `judged_inference_vlm.json`.
...
## 3.4 Qualitative Analysis: Visual Grounding Investigation

### Purpose

Accuracy alone indicates whether an answer is "correct" but not "why." A VLM may give a correct answer by genuinely reading the image (grounding), copying text, or guessing — accuracy is the same in all three cases, but the scientific significance is not. This analysis uses CoT reasoning chains as a proxy for grounding to assess whether the model truly used the image when it gave the correct answer.

### Two-Pronged Argument

1. **Quantitative hard evidence (zero additional cost):** The image-only accuracy — particularly the 13% for inference questions — is itself lower-bound evidence of very weak visual grounding. The existing image-only results in `results/compare_out/summary_vlm_ablation.json` are reinterpreted as a grounding measure without any new data.
2. **Qualitative supplement (this analysis):** 40 CoT traces from correctly answered cases were read and annotated to assess how many actually referenced image observations consistent with the gold standard.

### Data

- **File:** `results/grounding_annotation_completed.csv` (40 rows, labeled) + `code/analysis/sample_grounding_traces.py` (seed=42).
- **Composition:** 20 yes/no + 20 inference questions, all from Qwen2.5-VL CoT outputs, all from the correct-answer subset.
  - Yes/no: selected where `correct=True`; mostly image-only (14) + a small number of image+text (6).
  - Inference: all image+text (20, the condition with the highest risk of text-shortcut); full reasoning chains sourced from `combined_findings_results.json`'s `raw_output` (averaging ~1,300 characters, complete four-step chains).

### Annotation Scheme

| Label | Meaning |
|---|---|
| **A — True Visual Grounding** | The reasoning explicitly describes image findings consistent with the gold standard |
| **B — Textual Restatement / Vagueness** | Merely restates the MR findings text or uses clichés; no specific image observations |
| **C — Hallucination / Inconsistency** | Claimed findings contradict the gold standard (invented findings or incorrect location/severity) |

> **Key to interpretation:** Examine Steps 1–4 of the reasoning chain; do not focus solely on the `[Answer]` conclusion. Grounding is assessed from the *process*, not the final label.

### Execution

### Key Numerical Output

Among the correctly answered VLM responses, what percentage of reasoning chains contained image observations consistent with the gold standard (A) vs. were primarily driven by text paraphrasing (B)?

> *"Among the correctly answered VLM responses, only approximately X% of the reasoning chains contained image observations consistent with the gold standard (A), while the rest were primarily driven by text paraphrasing (B) → High accuracy overestimates true visual grounding."*
