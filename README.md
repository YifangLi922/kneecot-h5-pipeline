...
**Measured judge reliability.** Across the 136 manually-adjudicated items in `full_human_review_LLM.csv` / `full_human_review_VLM.csv` (the full `random_correct_spotcheck` sample plus a stratified sample of `boundary_case` items), human reviewers agreed with the judge on 132/136 (**97.06%**, Cohen's κ = **0.94**). The 4 disagreements (1 LLM, 3 VLM, all judge-said-`correct`/human-said-`incorrect`) were corrected directly in `judged_inference_llm.json` / `judged_inference_vlm.json`; the resulting accuracy shift is small (LLM CoT 51.0%→50.5%, VLM CoT_findings 60.0%→58.5%) and does not change any RQ1/RQ2 significance conclusion (all p-values remain <1e-6). This reliability check was run on the main matched (`_findings`) comparison only — the image-only ablation (§3.3) reuses the same judge/rubric without a separate human-adjudicated sample, which is a noted scope limitation rather than an independently validated condition.
